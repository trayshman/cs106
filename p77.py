...
def select(desc,name,collumn):
    good=[] #empty list
    for i in desc: # goes through and if the specified value in the specified collumn = the second parameter
        if i[collumn] == name:
            good.append(i) #its added to the list of rows

    return good #and returns it
...
def project(a,collumn):
    ...
    goodlist=[] #is a list of lists, this is what the function will return

    for row in a: # for each row in the Database
        # The variable row is a row from the database
        goodrow=[] # Each row of the result
        for i in collumn: # for each collumn(integer) in the list of collumns

            goodrow.append(row[i]) #append it to goodrow
        goodlist.append(goodrow) #which is then appended to the list of lists



    return goodlist #and returned
...
def select_like(desc,strung,collumn):
    good=[] #empty list
    for i in desc: #for each item in database
        if strung in i[collumn]: #if the string matches in the specified collum
            good.append(i) #added to the list of rows

    return good #returns the list of rows
